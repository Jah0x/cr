...
Платформа: для `/api/v1/platform/*` требуется платформенный токен от `POST /api/v1/platform/auth/login` (логин — `FIRST_OWNER_EMAIL`/`FIRST_OWNER_PASSWORD`). Платформенные запросы дополнительно ограничены `PLATFORM_HOSTS`.
