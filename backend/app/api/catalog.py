...
router = APIRouter(prefix="", tags=["catalog"], dependencies=[Depends(get_current_user)])


@router.get("/categories", response_model=list[CategoryOut])
async def list_categories(session: AsyncSession = Depends(get_db_session)):
    service = CatalogService(CategoryRepo(session), BrandRepo(session), ProductLineRepo(session), ProductRepo(session))
...
async def list_products(category_id: str | None = None, brand_id: str | None = None, line_id: str | None = None, q: str | None = None, active: bool | None = None, session: AsyncSession = Depends(get_db_session)):
    service = CatalogService(CategoryRepo(session), BrandRepo(session), ProductLineRepo(session), ProductRepo(session))
    filters = {"category_id": category_id, "brand_id": brand_id, "line_id": line_id, "q": q, "active": active}
