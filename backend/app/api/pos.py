...
def get_service(session: AsyncSession):
    return SalesService(SaleRepo(session), SaleItemRepo(session), PaymentRepo(session), StockRepo(session), StockBatchRepo(session), PaymentGateway())


@router.post("/sales", response_model=SaleOut)
async def create_sale(payload: SaleCreate, session: AsyncSession = Depends(get_db_session)):
...
async def add_sale_item(sale_id: str, payload: SaleItemCreate, session: AsyncSession = Depends(get_db_session)):
...
async def update_sale_item(sale_id: str, item_id: str, payload: SaleItemUpdate, session: AsyncSession = Depends(get_db_session)):
    service = get_service(session)
...
async def add_payment(sale_id: str, payload: PaymentCreate, session: AsyncSession = Depends(get_db_session)):
...
async def finalize_sale(sale_id: str, session: AsyncSession = Depends(get_db_session)):
...
async def void_sale(sale_id: str, session: AsyncSession = Depends(get_db_session)):
