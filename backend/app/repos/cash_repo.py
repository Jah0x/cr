...
class CashRegisterRepo:
    ...
    async def get_active(self):
        result = await self.session.execute(select(CashRegister).where(CashRegister.is_active.is_(True)))
        return result.scalars().all()

    async def get_by_id(self, register_id):
        result = await self.session.execute(select(CashRegister).where(CashRegister.id == register_id))
