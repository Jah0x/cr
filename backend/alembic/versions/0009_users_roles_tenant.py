...
def upgrade() -> None:
    ...
    op.add_column("users", sa.Column("tenant_id", postgresql.UUID(as_uuid=True), nullable=True), if_not_exists=True)
    op.add_column("roles", sa.Column("tenant_id", postgresql.UUID(as_uuid=True), nullable=True), if_not_exists=True)
    op.add_column("user_roles", sa.Column("tenant_id", postgresql.UUID(as_uuid=True), nullable=True), if_not_exists=True)
    ...
    op.create_index("ix_users_tenant_id", "users", ["tenant_id"], unique=False, if_not_exists=True)
    op.create_index("ix_users_email_tenant", "users", ["email", "tenant_id"], unique=True, if_not_exists=True)
    op.create_index("ix_roles_tenant_id", "roles", ["tenant_id"], unique=False, if_not_exists=True)
    op.create_index("ix_roles_name_tenant", "roles", ["name", "tenant_id"], unique=True, if_not_exists=True)
    op.create_index("ix_user_roles_tenant_id", "user_roles", ["tenant_id"], unique=False, if_not_exists=True)


def downgrade() -> None:
    conn = op.get_bind()
    op.drop_index("ix_user_roles_tenant_id", table_name="user_roles", if_exists=True)
    op.drop_index("ix_roles_name_tenant", table_name="roles", if_exists=True)
    op.drop_index("ix_roles_tenant_id", table_name="roles", if_exists=True)
    op.drop_index("ix_users_email_tenant", table_name="users", if_exists=True)
    op.drop_index("ix_users_tenant_id", table_name="users", if_exists=True)

    if _constraint_exists(conn, "fk_user_roles_tenant_id"):
        op.drop_constraint("fk_user_roles_tenant_id", "user_roles", type_="foreignkey")
    if _constraint_exists(conn, "fk_roles_tenant_id"):
        op.drop_constraint("fk_roles_tenant_id", "roles", type_="foreignkey")
    if _constraint_exists(conn, "fk_users_tenant_id"):
        op.drop_constraint("fk_users_tenant_id", "users", type_="foreignkey")

    op.drop_column("user_roles", "tenant_id", if_exists=True)
    op.drop_column("roles", "tenant_id", if_exists=True)
    op.drop_column("users", "tenant_id", if_exists=True)
