...
def upgrade() -> None:
    sale_status_enum = sa.Enum("completed", "void", name="salestatus")
    sale_status_enum.create(op.get_bind(), checkfirst=True)
    sale_status = sa.Enum("completed", "void", name="salestatus", create_type=False)
    op.create_table(
        "sales",
    ...
    op.add_column(
        "stock_moves",
        sa.Column("delta_qty", sa.Numeric(12, 3), server_default="0", nullable=False),
        if_not_exists=True,
    )
    op.add_column("stock_moves", sa.Column("ref_id", postgresql.UUID(as_uuid=True)), if_not_exists=True)
    op.add_column(
        "stock_moves",
        sa.Column("created_by_user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="SET NULL")),
        if_not_exists=True,
...
def downgrade() -> None:
    op.drop_column("stock_moves", "created_by_user_id", if_exists=True)
    op.drop_column("stock_moves", "ref_id", if_exists=True)
    op.drop_column("stock_moves", "delta_qty", if_exists=True)
    op.drop_index("ix_sale_items_product_id", table_name="sale_items", if_exists=True)
    op.drop_index("ix_sale_items_sale_id", table_name="sale_items", if_exists=True)
    op.drop_table("sale_items", if_exists=True)
    op.drop_index("ix_sales_status", table_name="sales", if_exists=True)
    op.drop_table("sales", if_exists=True)
