...
Таблица `tenants` находится в схеме `public`. Все остальные бизнес-таблицы живут в схеме арендатора, поэтому `tenant_id` в них не хранится — контекст арендатора задаётся через `search_path`.

## users
- `id` — UUID пользователя, первичный ключ; используется как стабильный идентификатор во всех связях (roles, sales, refunds).
- `email` — логин пользователя; используется при аутентификации и для отображения в админке, уникален в пределах схемы арендатора.
...
## tenants
- `id` — UUID арендатора, первичный ключ; используется в административном контуре.
...
- `sku` — артикул/код товара; используется для поиска и уникальности в пределах схемы арендатора.
...
- `receipt_id` — идентификатор чека у провайдера (уникален в рамках схемы арендатора).
