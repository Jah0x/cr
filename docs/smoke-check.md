...
4. Open `/invite/<token>` using the invite token and confirm:
   - `GET /api/v1/invitations/<token>` returns 200 with email/tenant/role.
   - `POST /api/v1/invitations/<token>/accept` accepts the token and sets a password.
