...
function TenantCard({ tenant }: { tenant: Tenant }) {
  ...
  const [userRoles, setUserRoles] = useState('manager')
  ...
  const inviteUserMutation = useMutation({ mutationFn: async () => (await api.post<TenantInvite>(`/platform/tenants/${tenant.id}/invite`, { email: userEmail, role_name: roleList[0] || 'manager' })).data, onSuccess: (data) => { setUserInviteResult(data); addToast(t('common.created'), 'success') }, onError: (error) => addToast(getApiErrorMessage(error, t, 'common.error'), 'error') })
